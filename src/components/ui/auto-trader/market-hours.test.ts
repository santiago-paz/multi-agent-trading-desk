...
describe('isMarketOpen', () => {
  ...
  describe('weekdays outside market hours', () => {
    it('returns closed before open (10:59 ART)', () => {
      ...
      expect(result.reason).toContain('abre a las 11:00');
    });

    it('returns closed at market close (17:00 ART)', () => {
      const result = isMarketOpen(artDate(2026, 4, 20, 17, 0));
      expect(result.open).toBe(false);
      expect(result.reason).toContain('cerró a las 17:00');
    });
  ...
  describe('weekends', () => {
    it('returns closed on Saturday', () => {
      ...
      expect(result.open).toBe(false);
      expect(result.reason).toBe('fin de semana');
    });
  ...
  describe('edge cases', () => {
    ...
    it('reason includes current ART time when open', () => {
      const result = isMarketOpen(artDate(2026, 4, 20, 14, 30));
      expect(result.reason).toContain('14:30 ART');
    });
  });
});
