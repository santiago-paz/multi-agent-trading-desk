...
export function MohnishPabraiDetail({ reasoning }: { reasoning: string }) {
  const t = useAutoTraderT();

  if (!reasoning || typeof reasoning !== 'string') return null;

  return (
    <div key="mohnish-pabrai-analysis" style={{ marginTop: 6, marginBottom: 8 }}>
      <strong>{t('detail.mohnish.title')}</strong>
      <div style={{
        display: 'flex',
        gap: '8px',
        marginTop: '6px',
        ...
        <AgentAvatar agent="mohnish_pabrai" size={48} />
