...
export function PhilFisherDetail({ reasoning }: { reasoning: string }) {
  const t = useAutoTraderT();

  if (!reasoning || typeof reasoning !== 'string') return null;

  return (
    <div key="phil-fisher-analysis" style={{ marginTop: 6, marginBottom: 8 }}>
      <strong>{t('detail.phil.title')}</strong>
      <div style={{
        display: 'flex',
        gap: '8px',
        marginTop: '6px',
        ...
        <AgentAvatar agent="phil_fisher" size={48} />
